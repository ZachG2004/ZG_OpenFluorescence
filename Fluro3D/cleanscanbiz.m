function [eem_blanc,maxpos,eem_cor,correct,eem_filter] = cleanscanbiz(eem,blanc,correct,parms,baseopt)
...
maxposH1 = [330+exmaxH1(end).*5 440+emmaxH1(end)*2 max(max(eemH1))];

%Extrait la matrice des intensité de fluorescence de H2
flH2 = eem_cor(95:102,8:11);
eemH2 = [];
eemH2(1:8,1:4) = flH2;
%Extrait l'excitation, l'emission et l'intensité fluorescente du pic max de
%la MODH2
maxposH2 = [];
maxH2 = max(max(eemH2)); % Valeur maximale de eemH2: intensité maximale de fluorescence.
...
if eemH2(i,j) == maxH2 % Si un élément de eemH2 = max(eemH2), alors il le place dans une autre matrice
...
maxposH2 = [245+exmaxH2(end).*5 434+emmaxH2(end)*2 max(max(eemH2))];

%Extrait la matrice des intensité de fluorescence de M
flM = eem_cor(67:87,16:22);
eemM = [];
eemM(1:21,1:7) = flM;
%Extrait l'excitation, l'emission et l'intensité fluorescente du pic max de
%la MODM
maxposM = [];
maxM = max(max(eemM)); % Valeur maximale de eemM: intensité maximale de fluorescence.
exmaxM = []; % Matrice vide
emmaxM = [];
k = 1;
l = 1;
disp(' ')
for i = 1:size(eemM,1) % Boucle parcourant chaque élément de eemM
for j = 1:size(eemM,2)
if eemM(i,j) == maxM % Si un élément de eemH2 = max(eemM), alors il le place dans une autre matrice
emmaxM(k) = i; % Cette matrice-là
k=k+1;
exmaxM(l) = j;
j=j+1;
end
...
maxposM = [285+exmaxM(end).*5 378+emmaxM(end)*2 max(max(eemM))];

%Extrait la matrice des intensité de fluorescence du P1
flP1 = eem_cor(61:66,5:6);
eemP1 = [];
eemP1(1:6,1:2) = flP1;
%Extrait l'excitation, l'emission et l'intensité fluorescente du pic max du
%P1
maxposP1 = [];
maxP1 = max(max(eemP1)); % Valeur maximale de eemP1: intensité maximale de fluorescence.
...
if eemP1(i,j) == maxP1 % Si un élément de eemP1 = max(eemP1), alors il le place dans une autre matrice
...
maxposP1 = [230+exmaxP1(end)*5 366+emmaxP1(end)*2 max(max(eemP1))];

%Extrait la matrice des intensité de fluorescence du Tryptophane-Like 1
flTrp1 = eem_cor(42:52,13:16);
eemTrp1 = [];
eemTrp1(1:11,1:4) = flTrp1;
%Extrait l'excitation, l'emission et l'intensité fluorescente du pic max du
%Trp-Like 1
maxposTrp1 = [];
maxTrp1 = max(max(eemTrp1)); % Valeur maximale de eemTrp1: intensité maximale de fluorescence.
exmaxTrp1 = []; % Matrice vide
emmaxTrp1 = [];
k = 1;
l = 1;
disp(' ')
for i = 1:size(eemTrp1,1) % Boucle parcourant chaque élément de eemTrp1
for j = 1:size(eemTrp1,2)
if eemTrp1(i,j) == maxTrp1 % Si un élément de eemTrp1 = max(eemTrp1), alors il le place dans une autre matrice
emmaxTrp1(k) = i; % Cette matrice-là
k=k+1;
exmaxTrp1(l) = j;
j=j+1;
end
...
maxposTrp1 = [270+exmaxTrp1(end).*5 328+emmaxTrp1(end)*2 max(max(eemTrp1))];

%Extrait la matrice des intensité de fluorescence du Tryptophane-Like 2
flTrp2 = eem_cor(42:52,4:6);
eemTrp2 = [];
eemTrp2(1:11,1:3) = flTrp2;
%Extrait l'excitation, l'emission et l'intensité fluorescente du pic max du
%Trp-Like 2
maxposTrp2 = [];
maxTrp2 = max(max(eemTrp2)); % Valeur maximale de eemTrp2: intensité maximale de fluorescence.
exmaxTrp2 = []; % Matrice vide
emmaxTrp2 = [];
k = 1;
l = 1;
disp(' ')
for i = 1:size(eemTrp2,1) % Boucle parcourant chaque élément de eemTrp2
for j = 1:size(eemTrp2,2)
if eemTrp2(i,j) == maxTrp2 % Si un élément de eemTrp2 = max(eemTrp2), alors il le place dans une autre matrice
emmaxTrp2(k) = i; % Cette matrice-là
k=k+1;
exmaxTrp2(l) = j;
j=j+1;
end
...
maxposTrp2 = [225+exmaxTrp2(end).*5 328+emmaxTrp2(end)*2 max(max(eemTrp2))];

%Extrait la matrice des intensité de fluorescence du Tyrosine-Like 1
flTyr1 = eem_cor(27:37,10:16);
eemTyr1 = [];
eemTyr1(1:11,1:7) = flTyr1;
%Extrait l'excitation, l'emission et l'intensité fluorescente du pic max du
%Tyr-Like 1
maxposTyr1 = [];
maxTyr1 = max(max(eemTyr1)); % Valeur maximale de eemTyr1: intensité maximale de fluorescence.
exmaxTyr1 = []; % Matrice vide
emmaxTyr1 = [];
k = 1;
l = 1;
disp(' ')
for i = 1:size(eemTyr1,1) % Boucle parcourant chaque élément de eemTyr1
for j = 1:size(eemTyr1,2)
if eemTyr1(i,j) == maxTyr1 % Si un élément de eemTyr1 = max(eemTyr1), alors il le place dans une autre matrice
emmaxTyr1(k) = i; % Cette matrice-là
k=k+1;
exmaxTyr1(l) = j;
j=j+1;
end
...
maxposTyr1 = [255+exmaxTyr1(end).*5 298+emmaxTyr1(end)*2 max(max(eemTyr1))];

%Extrait la matrice des intensité de fluorescence du Tyrosine-Like 2
flTyr2 = eem_cor(27:37,10:16);
eemTyr2 = [];
eemTyr2(1:11,1:7) = flTyr2;
%Extrait l'excitation, l'emission et l'intensité fluorescente du pic max du
%Tyr-Like 2
maxposTyr2 = [];
maxTyr2 = max(max(eemTyr2)); % Valeur maximale de eemTyr2: intensité maximale de fluorescence.
exmaxTyr2 = []; % Matrice vide
emmaxTyr2 = [];
k = 1;
l = 1;
disp(' ')
for i = 1:size(eemTyr2,1) % Boucle parcourant chaque élément de eemTyr2
for j = 1:size(eemTyr2,2)
if eemTyr2(i,j) == maxTyr2 % Si un élément de eemTyr2 = max(eemTyr2), alors il le place dans une autre matrice
emmaxTyr2(k) = i; % Cette matrice-là
k=k+1;
exmaxTyr2(l) = j;
j=j+1;
end
...
maxposTyr2 = [255+exmaxTyr2(end).*5 298+emmaxTyr2(end)*2 max(max(eemTyr2))];

maxpos = [maxposH1 0 maxposH2 0 maxposM 0 maxposP1 0 maxposTrp1 0 maxposTrp2 0 maxposTyr2 0];
